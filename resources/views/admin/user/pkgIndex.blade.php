                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @foreach ($users as $user)
                                        <tr>
                                            ...
                                            <td>{{$user->username}}</td>
                                            <td>{{$user->password}}</td>
                                            <td><a href="{{url('/admin/manage/admin/pkg/'.$user->id.'/edit')}}" class="btn btn-warning">Edit</a>
                                                <a href="#deleteAdmin{{$user->id}}" class="btn btn-danger">DELETE</a></td>
                                        </tr>
                                        <div data-remodal-id="deleteAdmin{{$user->id}}" role="dialog" class="delete_model_c">
                                            <div>
                                                <h2>Delete PKG Admin</h2>
                                                <p>The selected PKG Admin will be DELETED. This action CANNOT be reversed. Are you sure?</p>
                                            </div>
                                            <br>
                                            <form method="post" action="{{url('/admin/manage/admin/pkg/'.$user->id)}}">
                                                @csrf @method('DELETE')
